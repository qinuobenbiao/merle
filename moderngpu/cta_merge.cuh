...
// Load a halo element conditionally in sorted search with flags
template <int nt, int vt, typename type_t>
MGPU_DEVICE array_t<type_t, vt + 1>
load_two_streams_reg_halo(const type_t *a, int a_count, const type_t *b,
                          int b_count, int tid) {
  b -= a_count;
  array_t<type_t, vt + 1> x;
  strided_iterate<nt, vt>([&](int i, int index) {
    const type_t* p = (index >= a_count) ? b : a;
    x[i] = p[index];
  }, tid, a_count + b_count);
...
MGPU_DEVICE enable_if_t<
  !(std::is_pointer<a_it>::value && std::is_pointer<b_it>::value), 
  array_t<type_t, vt + 1> 
> load_two_streams_reg_halo(a_it a, int a_count, b_it b, int b_count, int tid) {
  b -= a_count;
  array_t<type_t, vt + 1> x;
  strided_iterate<nt, vt>([&](int i, int index) {
    x[i] = (index < a_count) ? a[index] : b[index];
  ...
  int index = nt * vt + tid;
  if (index < a_count + b_count)
    x[vt] = (index < a_count) ? a[index] : b[index];
  // else x[vt] = -2;
  return x;
}
