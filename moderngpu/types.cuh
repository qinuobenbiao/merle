...
struct size_array_t {
  ...
  MGPU_HOST_DEVICE type_t operator[](int i) const { return data[i]; }
  MGPU_HOST_DEVICE type_t& operator[](int i) { return data[i]; }
  MGPU_HOST_DEVICE void push(type_t x) { data[cur_size++] = std::move(x); }
  MGPU_HOST_DEVICE void pop() { --cur_size; }

  size_array_t() = default;
  size_array_t(const size_array_t&) = default;
  size_array_t& operator=(const size_array_t&) = default;
};
