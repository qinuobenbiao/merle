...
template<int nt, int vt, int vt0 = vt, bool nochk = false,
         typename type_t, typename it_t>
MGPU_DEVICE void
reg_to_mem_strided(array_t<type_t, vt> x, int tid, int count, it_t mem) {

  strided_iterate<nt, vt, vt0, nochk>([=](int i, int j) { 
    mem[j] = x[i]; 
  }, tid, count);
}

template<int nt, int vt, int vt0 = vt, bool nochk = false, typename it_t>
