...
struct cta_scan_t {
  ...
  template<int vt, int vt0 = 0, typename op_t = plus_t<type_t> >
  ...
  scan(int tid, array_t<type_t, vt> x, storage_t& storage, 
    ...
    if constexpr (vt0 > 0)
      iterate<vt0>([&](int i) { x[i] = i ? op(x[i], x[i - 1]) : x[i]; });
    if (count >= nt * vt) {
      iterate<vt0, vt>([&](int i) { x[i] = i ? op(x[i], x[i - 1]) : x[i]; });
    } else {
      iterate<vt0, vt>([&](int i) {
        int index = vt * tid + i;
        type_t lhs = index < count ? x[i] : init;
        x[i] = i ? op(lhs, x[i - 1]) : lhs;
      });
    }
