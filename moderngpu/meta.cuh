...
// Invoke unconditionally for first vt0 iterations and checks range after vt0.
template<int nt, int vt, int vt0 = vt, bool nochk = false, typename func_t>
MGPU_DEVICE void strided_iterate(func_t f, int tid, int count) {
  // Unroll the first vt0 elements of each thread.
  if (vt0 > 1 && (nochk || count >= nt * vt0)) {
